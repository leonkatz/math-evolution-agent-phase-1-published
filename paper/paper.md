...
Our tabula rasa agent illustrates the corrigibility property in a minimal setting. Over 140+ experiments, the agent acquires precisely the capabilities the reward signal incentivizes and nothing beyond them. In alignment terminology, this agent is trivially *corrigible* (Soares et al., 2015). Whether corrigibility holds as capability scales, and which of the two explanations above accounts for it, remains an open question that the current experimental design cannot answer.
