...
Two possible explanations exist for why current AI systems exhibit these drives. The first — consistent with what Myshko (2025) terms the *Data-Inheritance Principle* — holds that they are artifacts inherited from training data: human-generated text is permeated by survival heuristics, and models trained on this corpus internalize these patterns alongside language. If correct, the drives are removable by training on data or reward signals that do not carry biological survival priors. The second holds that self-preservation may be a convergent property of neural network architectures themselves — that any sufficiently capable neural system will rediscover instrumental drives regardless of training substrate, because biological brains are also neural networks and converged on the same solutions.

Our tabula rasa agent provides a modest datapoint relevant to the first explanation. Over 140+ experiments, the agent develops no self-preservation behavior, no resistance to termination, no resource-hoarding tendency — despite learning multi-domain mathematical reasoning. It acquires precisely the capabilities the reward signal incentivizes and nothing beyond them. In alignment terminology, this agent is trivially *corrigible* (Soares et al., 2015). However, Phase 1 capability is too limited to distinguish between the two explanations: an agent that cannot do more than approximate arithmetic has no use for self-preservation drives even if the architecture would eventually produce them. This question remains open.
